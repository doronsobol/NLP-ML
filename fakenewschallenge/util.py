...
def pipeline_train(train, test, lim_unigram, use_cosine=True, just_cosine=False):
    ...
    for instance in train.instances:
        ...
        if just_cosine:
            #pdb.set_trace()
            feat_vec = np.array([np.squeeze(np.array([tfidf_cos]))])
        elif use_cosine:
...
def pipeline_test(test, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer, use_cosine=True, just_cosine=False):
    ...
    for instance in test.instances:
        ...
        if (head, body_id) not in cos_track:
            ...
            tfidf_cos = cos_track[(head, body_id)]
        if just_cosine:
            #feat_vec = np.squeeze(np.array([tfidf_cos]))
            feat_vec = np.array([np.squeeze(np.array([tfidf_cos]))])
            #feat_vec = np.array([tfidf_cos])
        elif use_cosine:
            feat_vec = np.squeeze(np.c_[head_tf, body_tf, tfidf_cos])
        else:
            feat_vec = np.squeeze(np.c_[head_tf, body_tf])
