...
def create_csv_from_files(files_agree, files_disagree, files_discuss, name):
    with open(name+'data.csv', 'w', encoding="utf8") as articles_file, open(name+'_stances.csv', 'w', encoding="utf8") as stances_file, open(name+'_stances_unlabeled.csv', 'w', encoding="utf8") as unlabeled_file:
        articles_fieldnames = ['head','body', 'stance']
        ...
        for i in range(len(stances)):
            articles_writer.writerow({'stance': labels[i], 'head': stances[i], 'body': bodies[i]})
